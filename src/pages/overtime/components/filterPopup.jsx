...
const FilterOvertimePopup = ({ isFilterPopup, closeFilterPopup, applyFilter, tempSelectedStatus, setTempSelectedStatus, tempSelectedType, setTempSelectedType, tempSelectedPeriod, setTempSelectedPeriod, toggleSelection, theme, dataLeaveTypes }) => {
    const language = useSelector(selectLanguages)

    const typeData = dataLeaveTypes.map(item => ({
        type: item.name,
        label: item.name
    }));
            ...
            <Block style={{ height: "95%", padding: '15px', margin: 0, color: theme === "light" ? "black" : "white", display: "flex", justifyContent: "space-between", flexDirection: "column" }}>
                <div>
                    ...
                    <div style={{ height: "69vh", overflowY: "auto" }}>
                        <p style={{ fontSize: "var(--font-md)", fontWeight: 700 }}>{translate('overtime_type', language)}</p>
...
export default FilterOvertimePopup