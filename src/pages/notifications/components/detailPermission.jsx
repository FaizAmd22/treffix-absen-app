...
const DetailPermission = ({ item, leaveType, handleOpenPopup }) => {
    const language = useSelector(selectLanguages)

    return (
        <div style={{ display: "grid", gridTemplateColumns: "60% 40%", fontWeight: 400, marginTop: "10px", marginBottom: "20px" }}>
            ...
            <p style={{ fontWeight: 700, textAlign: "end", margin: "10px 0", textTransform: "capitalize" }}>{leaveType}</p>

            <p style={{ margin: "10px 0" }}>{translate('start_date', language)}</p>
            <p style={{ fontWeight: 700, textAlign: "end", margin: "10px 0" }}>{formatDate(item.start_date, language, "with-weekdays")}</p>

            <p style={{ margin: "10px 0" }}>{translate('end_date', language)}</p>
            <p style={{ fontWeight: 700, textAlign: "end", margin: "10px 0" }}>{formatDate(item.end_date, language, "with-weekdays")}</p>
